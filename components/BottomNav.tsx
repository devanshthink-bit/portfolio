...
// Solar moon-linear: a plain crescent (Devansh, 17 Sep 2026: "make this normal moon icon").
  ...
  return (
    ...
    >
      ...
      {/* Phones: one hamburger at the right; its panel holds the four options (Devansh, 15 Sep).
          The click sound plays on pointer-up: iPhone Safari only lets a finger lifting unlock audio,
          so on pointer-down it stayed silent (Devansh, 26 Sep 2026). */}
      <button className="dock-burger" aria-label={menu ? "Close menu" : "Menu"} aria-expanded={menu} onPointerUp={() => playDockClick(1)} onClick={() => setMenu((m) => !m)}>
        <Solar body={menu ? CROSS : BURGER} />
      </button>
      <div className={`dock-menu${menu ? " open" : ""}`} aria-hidden={!menu}>
        <Link href="/#recent-work" tabIndex={menu ? 0 : -1} onPointerUp={() => playDockClick(1.06)} onClick={(e) => {
          setMenu(false);
          if (pathname !== "/") return;
          e.preventDefault();
        ...
        <Link href="/about" tabIndex={menu ? 0 : -1} onPointerUp={() => playDockClick(1.12)} onClick={() => setMenu(false)}>About</Link>
        <Link href="/resume" tabIndex={menu ? 0 : -1} onPointerUp={() => playDockClick(1.19)} onClick={() => setMenu(false)}>Resume</Link>
        <button tabIndex={menu ? 0 : -1} aria-label={isDark ? "Light mode" : "Dark mode"} onPointerUp={() => playDockClick(1.26)} onClick={(e) => toggleTheme(e.currentTarget, () => {
