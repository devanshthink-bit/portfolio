...
**Apple look (19 Sep 2026)**: Liquid Glass only on floating navigation (tab bar capsule, bar buttons, toast) via effect/glass. Sheets are solid color/background/grouped, like the page. SF Symbols only (SF/… components), outline for unselected tabs, fill for selected. Tab screens use large titles (AppHeader Type=Large); inner screens the bold centred title. Every screen: color/background/grouped page (#F2F2F7), white cards and white list groups, no shadows; the top bar has no fill. Lists (Messages, Notifications, Profile) are one white rounded group with hairline dividers. Neutral tags and control tracks: color/fill/tertiary. Corner smoothing 60% everywhere. Settings-style lists are one group with hairline dividers.
...
Warning text, icons and lines are one colour: #C93400 (Apple high-contrast orange, 4.7–5.3:1). Tokens color/text/warningAA and color/icon/warning both point to it.
Every tappable control has a 44pt tap area (20 Sep 2026). Small in-row actions (Update, Mark it) use Button Type=Secondary Small: 28pt visible, with a "Hit area 44" frame; they sit on the line of the thing they act on (the status tag).
Section and group counts go in the title in brackets, e.g. "Lower match (2)" (20 Sep 2026). Unread counts are the blue dot at the row's right.
