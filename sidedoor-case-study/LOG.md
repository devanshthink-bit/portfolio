...
CHANGE · 2026-09-24 · Case study · Story format reverted; hero screens; two export bugs
Asked:     Devansh didn't like the story version ("make it like it was before"); wanted the case study's top screens the same as the home card; asked why the back button's shadow was cut, and why the skill sheet had black corners.
Changed:   Reverted acf76a6, so the page is back to the version with sections. Kept later changes. The learning "Check where the research came from" (it hinted at the AI interviews) is replaced with "Write down what will break before testing. I guessed 1 of 13." Hero is Job · Login · Request, like the card.
Found:     Both bugs were in my exported images only. Figma: the back button's 40 blur shadow isn't clipped (no filled layer sits over any V6 header; the sheet frame has 32 corners and clips). Prototype: the bar sits above the content (z-index 25), and the sheet has real corners over a see-through scrim. So no Figma or prototype change.
  - Cut shadow: five crops (job, track, st_noanswer, st_cantsend, post) joined the header to content scrolled further down, and the join cut the shadow. Rebuilt: the header's own shadow band (from its export, divided by the page colour #FBFAF8) is multiplied onto the content under it.
  - Black corners: the sheet's export carried the States board's #3B3F46 behind its rounded corners. Rebuilt proof.webp with the corners cut out (1.5px inset, to drop the dark edge) before laying it over the dimmed request.

DECISION · 2026-09-24 · Research board (public/sidedoor-process.html)
Asked:     Devansh wanted a complete HTML of the design and research process, with the business and product thinking, like RedBus's artefacts page, in SideDoor's own theme, and "something else other than redbus's accordion".
Built:     One page in the app's design language (Inter, #FBFAF8 page, white cards with the app's shadow, blue only for actions, status tags by meaning). The process is laid out like the app's request tracker: a status timeline on the left fills as you read 15 stages, with "Stage n of 15" in the top bar. Stages: business lens (who pays, rejected models, quality-as-identity, North Star rings and checklist, vanity metrics), scope and the LinkedIn critique, research and bias line, the 91 notes (filter by person or side, each coloured by cluster), 5 clusters (each opens its notes), 5 jobs → 3, insight statements colour-coded by part, the problem statement with the rejected drafts and the chain back to n76 and n35, landscape matrix, where AI belongs, the brief and main path, the design → input → output → North Star chain with the guardrail and four growth loops, V2 to V6, the attack, and the kill conditions. Business frameworks follow Devansh's Product Anatomy slides (PAM04 L01–L04, PAM05 L03) via BUSINESS.md.
...
CHANGE · 2026-09-25 · Two of the four wording fixes reversed: the prototype was newer
Changed:   Checked the history after Devansh asked. Search with no results and the resume preview had changed in code after Figma (23 Sep: the iOS empty-state pass, and the per-candidate resume that evening), and Figma was never synced. So the code goes back to "No results for “Rahul”" + "Check the spelling or try a new search." and the resume with email, a Projects list, skills from the resume match and 2021. Figma now matches: the search screen uses the EmptyState component (magnifyingglass, no action), and the Resume Preview has the email line, Projects with bold titles, those skills and 2021. Both frames exist only on UI Screens.
